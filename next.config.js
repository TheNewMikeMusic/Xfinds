...
const nextConfig = {
  ...
  // Rewrites to handle icon.svg via API route
  async rewrites() {
    return [
      {
        source: '/icon.svg',
  ...
  async headers() {
    return [
      ...
      {
        // Static image cache headers (exclude icon.svg which is handled by Next.js)
      ...
      {
        // SVG files cache headers (excluding icon.svg)
        source: '/:path*\\.svg',
        headers: [
          {
            key: 'Cache-Control',
